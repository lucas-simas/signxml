...
class Assinar_XML_IPC_BRASIL(Resource):
	...
	def post(self):
		try:
			...
			signed_root = signxml.XMLSigner(method=signxml.methods.enveloped, digest_algorithm="sha1", signature_algorithm="rsa-sha1", c14n_algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315").sign(root, key=key, cert=cert)
			# verified_data = XMLVerifier().verify(signed_root, ca_pem_file=cert).signed_xml

			xmlstr = etree.tostring(signed_root, encoding='utf8', method='xml')

			# et = etree.ElementTree(signed_root)
			# et.write('output.xml', pretty_print=True)

			return jsonify({'status': True, 'code': None, 'data': {'xml': xmlstr}})
